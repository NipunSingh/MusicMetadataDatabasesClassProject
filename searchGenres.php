...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $select_q = "SELECT id, name FROM `music`.`genre` WHERE name LIKE '%".$query."%'";
    $genres = $conn->query($select_q);
    $result = "";
    if ($genres->num_rows > 0 && strcmp("", $query) != 0) {
        $result = "<ul class='no-bullet-list'>";
        while($row = $genres->fetch_assoc()) {
            $result = $result."<li><a href='addGenreFavorite.php?u_id=".$user_id."&g_id=".$row["id"]."'><span class='bold-name'>".$row["name"]."</span></a></li>";
        }
        $result = $result."</ul>";
    }
    $result = $result."";
